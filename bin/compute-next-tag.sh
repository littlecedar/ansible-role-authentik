...
#
# Tags look like `v<authentik version>-<release>`:
#
# - if defaults/main.yml points at an authentik version that has never been
#   released, the release counter restarts at 0 (`v2026.5.6-0`)
# - otherwise the counter is incremented (`v2026.5.6-1`), but only if something
...
# `authentik_version` is the leaf that Renovate rewrites, and the only place
# where the version appears as a literal. Everything else that mentions it
# (`authentik_container_image_tag`, `authentik_container_image`) is derived from
# it through Jinja and would yield a template expression rather than a version.
version="$(sed -nE 's|^authentik_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the authentik version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `2026.5.5`), but the tags
# do (`v2026.5.5-0`). Stripping any `v` before prepending one keeps this correct
